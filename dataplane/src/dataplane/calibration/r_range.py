...
__all__ = ["NodeClassThroughput", "RRange", "synthesizable", "synthesizable_range"]
...
def synthesizable(snapshots: list[dict[str, Any]]) -> tuple[float, float]:
    """R as a **range**, read straight off a set of C-3 snapshots.

    `synthesizable_range` works from campaign reports, which is what my own pipeline holds.
    This works from the snapshots themselves, which is what anyone on the far side of the
    seam holds: R can be recomputed from `contracts/cost_models/` without a copy of my run
    directories.

    **The two will not agree exactly, and that is not a bug in either.** They are different
    ...
    **The ratio is only taken at a cell every node class shares.** That is the whole reason
    to compute it here rather than divide two headline throughputs. Decode rate falls as the
    KV context grows, so a class measured at a 256-token prompt looks slower than the same
    class at 64, and comparing across different cells folds that workload difference into R
    where nothing downstream can separate it out again. This is not a hypothetical: it cost
    this study 20% of its ratio, reading 1.66x instead of 2.00x, because the fast node
    happened to have been calibrated at p256 and the slow one at p64. No common cell means
    no honest ratio, so this raises rather than quietly picking one.
    """
    latest: dict[str, dict[str, Any]] = {}
    for snap in sorted(snapshots, key=lambda s: (s["measured_at_unix"], s["snapshot_id"])):
        latest[snap["node_class"]] = snap
    if len(latest) < 2:
        raise ValueError(
            f"R is a ratio between two node classes; these snapshots describe {len(latest)} "
            f"({sorted(latest)}). Calibrate a second (-ngl, threads, parallel) point before "
            "asking for a range"
        )
    ...
    by_class = {
        ...
        for node_class, snap in latest.items()
    }
    common = set.intersection(*(set(cells) for cells in by_class.values()))
    if not common:
        raise ValueError(
            f"node classes {sorted(by_class)} share no calibrated cell, so a ratio between "
            "them would be part hardware difference and part workload difference with no way "
            "to tell them apart. Re-calibrate them on a common (prompt, output, concurrency) "
            "point"
        )
    r_max = max(
        max(by_class[c][cell] for c in by_class) / min(by_class[c][cell] for c in by_class)
        for cell in common
    )
    return 1.0, r_max
